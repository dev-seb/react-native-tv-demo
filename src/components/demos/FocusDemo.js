import React, {Component} from 'react';
import {
  View,
  Text,
  TouchableHighlight,
  StyleSheet
} from 'react-native';
import findNodeID from '../../findNodeID';
import Style from "../Style";

class FocusDemo extends Component {

  constructor(props) {
    super(props);
    // Init refs
    this.topLeftRef = React.createRef();
    this.bottomLeftRef = React.createRef();
    this.middleLeftRef = React.createRef();
    // Init state
    this.state = {
      topMiddleFocused: false,
      topRightPressed: false
    }
  }

  componentDidMount() {
    // Override next focus bottom for Top Left
    if(this.topLeftRef) {
      this.topLeftRef.setNativeProps(
        {nextFocusDown: findNodeID(this.bottomLeftRef)}
      );
    }
    // Override next focus top for Bottom Left
    if(this.bottomLeftRef) {
      this.bottomLeftRef.setNativeProps(
        {nextFocusUp: findNodeID(this.topLeftRef)}
      );
    }
    // Disable next focus left for Middle Left
    if(this.middleLeftRef) {
      this.middleLeftRef.setNativeProps(
        {nextFocusLeft: findNodeID(this.middleLeftRef)}
      );
    }
  }

  render() {
    return (
      <View style={Style.styles.right}>
        <View style={Style.styles.header}>
          <Text style={Style.styles.headerText}>{"Focus Demo"}</Text>
        </View>
        <View style={Style.styles.content}>
          <View style={styles.buttons}>
            <TouchableHighlight
              ref={ref => this.topLeftRef = ref}
              nativeID={'top_left_button'}
              onPress={()=>{}}
              style={styles.button}
              underlayColor={Style.buttonFocusedColor}>
              <View>
                <Text style={styles.buttonText}>{"Top Left"}</Text>
                <Text style={styles.buttonProps}>{"nextFocusDown={Bottom Left}"}</Text>
              </View>
            </TouchableHighlight>
            <TouchableHighlight
              onPress={()=>{}}
              onFocus={(e)=>{
                this.setState({topMiddleFocused: true})
              }}
              onBlur={(e)=>{
                this.setState({topMiddleFocused: false})
              }}
              style={[
                styles.button,
                this.state.topMiddleFocused && styles.buttonFocused
              ]}
              underlayColor={Style.buttonFocusedColor}>
              <View>
                <Text style={styles.buttonText}>{"Top Middle"}</Text>
                <Text style={styles.buttonProps}>{"onFocus: change style"}</Text>
              </View>
            </TouchableHighlight>
            <TouchableHighlight
              onPress={(e)=>{
                if(e.eventKeyAction !== undefined) {
                  this.setState({topRightPressed: parseInt(e.eventKeyAction) === 0});
                }
              }}
              style={[
                styles.button,
                this.state.topRightPressed && styles.buttonPressed
              ]}
              underlayColor={Style.buttonFocusedColor}>
              <View>
                <Text style={styles.buttonText}>{"Top Right"}</Text>
                <Text style={styles.buttonProps}>{"onPress: change style"}</Text>
              </View>
            </TouchableHighlight>
          </View>
          <View style={styles.buttons}>
            <TouchableHighlight
              nativeID={'middle_left_button'}
              ref={ref => this.middleLeftRef = ref}
              onPress={()=>{}}
              style={styles.button}
              underlayColor={Style.buttonFocusedColor}>
              <View>
                <Text style={styles.buttonText}>{"Middle Left"}</Text>
                <Text style={styles.buttonProps}>{"nextFocusLeft is disabled"}</Text>
              </View>
            </TouchableHighlight>
            <TouchableHighlight
              onPress={()=>{}}
              style={styles.button}
              hasTVPreferredFocus={true}
              underlayColor={Style.buttonFocusedColor}>
              <View>
                <Text style={styles.buttonText}>{"Middle Middle"}</Text>
                <Text style={styles.buttonProps}>{"hasTVPreferredFocus={true}"}</Text>
              </View>
            </TouchableHighlight>
            <TouchableHighlight
              onPress={()=>{}}
              style={styles.button}
              activeOpacity={0.5}
              underlayColor={Style.buttonFocusedColor}>
              <View>
                <Text style={styles.buttonText}>{"Middle Right"}</Text>
                <Text style={styles.buttonProps}>{"activeOpacity={0.5}"}</Text>
              </View>
            </TouchableHighlight>
          </View>
          <View style={styles.buttons}>
            <TouchableHighlight
              ref={ref => this.bottomLeftRef = ref}
              nativeID={'bottom_left_button'}
              onPress={()=>{}}
              style={styles.button}
              underlayColor={Style.buttonFocusedColor}>
              <View>
                <Text style={styles.buttonText}>{"Bottom Left"}</Text>
                <Text style={styles.buttonProps}>{"nextFocusUp={Top Left}"}</Text>
              </View>
            </TouchableHighlight>
            <TouchableHighlight
              disabled={true}
              onPress={()=>{}}
              style={styles.button}
              underlayColor={Style.buttonFocusedColor}>
              <View>
                <Text style={styles.buttonText}>{"Bottom Middle"}</Text>
                <Text style={styles.buttonProps}>{"disabled={true}"}</Text>
              </View>
            </TouchableHighlight>
            <TouchableHighlight
              onPress={()=>{}}
              style={styles.button}
              tvParallaxProperties={{enabled: true}}
              underlayColor={Style.buttonFocusedColor}>
              <View>
                <Text style={styles.buttonText}>{"Bottom Right"}</Text>
                <Text style={styles.buttonProps}>{"tvParallaxProperties={{enabled: true}}"}</Text>
              </View>
            </TouchableHighlight>
          </View>
        </View>
      </View>
    );
  }
}

export default FocusDemo;

const styles = StyleSheet.create({
  buttons: {
    flex: 1,
    flexDirection: 'row',
    justifyContent: 'center',
    alignItems: 'center',
  },
  button: {
    width: Style.px(400),
    height: Style.px(200),
    margin: Style.px(10),
    backgroundColor: Style.buttonUnfocusedColor,
    justifyContent: 'center',
    alignItems: 'center',
  },
  buttonFocused: {
    borderWidth: Style.px(10),
    borderColor: '#eb4034',
  },
  buttonPressed: {
    borderWidth: Style.px(10),
    borderColor: '#34eb5f',
  },
  buttonText: {
    fontSize: Style.px(30),
  },
  buttonProps: {
    fontSize: Style.px(15),
  }
});